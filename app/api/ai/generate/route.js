...
const buildPrompt = (digest, count) =>
  [
    ...
    ``,
    `IMPORTANT — preserve the docs' own structure. When a document presents an explicit set of`,
    `items meant to be worked through — questions to answer, a checklist, numbered sections,`,
    `prompts awaiting a response, or a list of areas — surface EACH of those items as its own`,
    `separate task (e.g. one task per question). Do NOT collapse such a set into a single`,
    `summary task. Keep the wording faithful to the source. Add a few synthesis/next-step`,
    `tasks after the extracted items if useful.`,
    ``,
    `Rules:`,
    ...
    `- Order them as the docs present them, then by impact.`,
    `- Keep each item under ~16 words.`,
    ``,
